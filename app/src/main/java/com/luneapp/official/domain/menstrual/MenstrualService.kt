...
data class MenstrualCycle(
    ...
    val isAnomaly: Boolean = false // 是否為異常短週期（少於 14 天）
)
...
class MenstrualService(private val repository: RecordsRepository) {
    ...
    fun getMenstrualCycle(
        ...
        // Mark whether this is an anomalously short cycle
        ...
        // Ovulation typically occurs 14 days before the next period (count-back method).
        // Luteal phase is relatively constant (~14 days).
        // Follicular phase length varies based on follicle maturation.
        ...
        val ovulationDates = buildSet {
            // For anomalous cycles the calculation is unreliable, but we still provide
            // a value so the UI layer can indicate the anomaly.
            // Ovulation fertile window: peak day ±4 days (total 6-day window).
            for (offset in -4..1) {
    ...
    private fun predictNextCycles(records: List<MenstrualRecord>, count: Int = 3, cycleLength: Int = 28): List<PredictedCycle> {
        // Calculate average historical cycle length, excluding records shorter than 14 days.
