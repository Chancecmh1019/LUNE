...
open class SettingsRepository(private val dataStore: DataStore<Preferences>) {
    ...
    // ---------- User status (medical condition) ----------

    suspend fun getUserStatus(): UserStatus =
        UserStatus.fromValue(dataStore.data.first()[USER_STATUS])

    suspend fun setUserStatus(value: UserStatus) {
        dataStore.edit { prefs -> prefs[USER_STATUS] = value.name }
    }
